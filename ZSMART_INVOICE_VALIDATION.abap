...
*&=====================================================================*
*& SECTION 2: TEST DATA SCENARIOS — ZINV_VALIDATION
*&=====================================================================*
*& Test data is loaded via FORM load_test_data below.
*& On the selection screen, tick the checkbox "Load Test Data"
*& the first time you run the program. Untick it on subsequent runs.
*&
...
SELECTION-SCREEN BEGIN OF BLOCK blk1 WITH FRAME TITLE TEXT-001.
  ...
  PARAMETERS:     p_load   TYPE abap_bool DEFAULT abap_false.
...
INITIALIZATION.
  ...
  TEXT-002 = 'Load test data into ZINV_VALIDATION (first run only)'.
...
*----------------------------------------------------------------------*
*& Form LOAD_TEST_DATA
*& Inserts 5 sample records into ZINV_VALIDATION.
*& Called only when "Load test data" checkbox is ticked on screen.
*& Run ONCE on first execution — untick on all subsequent runs.
*----------------------------------------------------------------------*
FORM load_test_data.
  ...
  IF sy-subrc = 0.
    COMMIT WORK.
    MESSAGE 'Test data loaded successfully. 5 records inserted.' TYPE 'S'.
  ELSE.
    ROLLBACK WORK.
    MESSAGE 'Test data load failed. Records may already exist.' TYPE 'W'.
  ENDIF.
ENDFORM.
